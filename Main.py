...
Security fixes applied:
  ...
  #8  Tokens over HTTPS → documented
  ...
  #12 Admin panel → admin can delete/resolve any post using ADMIN_SECRET env var
"""
...
ADMIN_SECRET        = os.getenv("ADMIN_SECRET", "changeme-admin-secret")  # set this in env!
...
def admin_list_items(
    ...
    """Admin: list ALL items including resolved, with full contact info."""
    ...
    conn.close()
    return {
        "items":  [dict(r) for r in rows],   # full contact info, no masking
        "total":  total,
        "page":   page,
...
def admin_resolve(item_id: str, request: Request, x_admin_secret: Optional[str] = Header(None)):
    """Admin: resolve any item without needing the edit token."""
    verify_admin(x_admin_secret)
    ip   = get_ip(request)
    conn = get_db()
...
def admin_verify(request: Request, x_admin_secret: Optional[str] = Header(None)):
    """Admin: verify the secret is correct."""
    verify_admin(x_admin_secret)
